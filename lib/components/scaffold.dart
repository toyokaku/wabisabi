    ...
    if (isDark) {
      ...
      for (int i = 0; i < size.width; i += 20) {
        for (int j = 0; j < size.height; j += 20) {
          ...
          if (noise > 0.7) {
            canvas.drawCircle(Offset(i + noise * 10, j + noise * 10),
                1 + noise * 3, paint);
          }
        }
      }
      ...
      for (int i = 0; i < 10; i++) {
        ...
        for (int j = 0; j < 5; j++) {
          path.quadraticBezierTo(
            startX + ((random + i + j * 2) % 150) - 75,
            startY + 25 + ((random + i + j * 3) % 50),
            startX + ((random + i + j) % 200) - 100,
            startY + 50 + ((random + i + j) % 100),
          );
        }
      ...
      for (int i = 0; i < size.width; i += 15) {
        for (int j = 0; j < size.height; j += 15) {
          final noise = ((i * j + random) % 100) / 100;
          if (noise > 0.6) {
            canvas.drawCircle(Offset(i + noise * 5, j + noise * 5),
                0.5 + noise * 1.5, paint);
          }
        }
      }
